...
import { addMessageToHistory, getHistoryByOrg } from "./chatHistory";
...
function generateOrganizationId() {
  ...
  // End of file: removed stray closing brace
  ...
  function extractAddress(contactText: string): string {
    ...
    const lines = contactText
      .split(/\n|\r/)
      .filter((l) =>
        /address|suite|drive|dr\.|blvd|road|rd\.|ave|avenue|street|st\./i.test(
          l
        )
      );
  ...
  async function askOpenAIAssistant(
    ...
    return (
      response.data.choices?.[0]?.message?.content || "No answer returned."
    );
  }
