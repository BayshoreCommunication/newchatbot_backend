...
exports.askOpenAIAssistant = askOpenAIAssistant;
const axios_1 = __importDefault(require("axios"));
const crypto_1 = __importDefault(require("crypto"));
const cheerio = require("cheerio");
// Helper to generate a mock organization ID
function generateOrganizationId() {
    return "org-" + crypto_1.default.randomBytes(12).toString("hex");
}
// Caching logic
...
function extractAddress(contactText) {
    ...
    const lines = contactText
        .split(/\n|\r/)
        .filter((l) => /address|suite|drive|dr\.|blvd|road|rd\.|ave|avenue|street|st\./i.test(l));
    return lines.length > 0 ? lines.join(" ") : "";
}
// Call OpenAI Chat Completions API (GPT-4o/4.5)
async function askOpenAIAssistant(apiKey, question, websiteContentRaw) {
